...
def _bullish_candles() -> list:
    ...
    # ts_candle is bearish (close < open): sweeps the fractal low → bias = BULLISH
    ...
    ts_candle = make_candle("2024-01-15 16:00:00", open=1.09950, high=1.10100, low=1.09700, close=1.09800)
    live = make_candle("2024-01-15 20:00:00")
    return [ref, swing_low, recovery, ts_candle, live]


def _bearish_candles() -> list:
    # fractal swing high at index 1; ts_candle is bullish (close > open): sweeps it → bias = BEARISH
    ...
    ts_candle = make_candle("2024-01-15 16:00:00", open=1.10100, high=1.10400, low=1.10050, close=1.10300)
    live = make_candle("2024-01-15 20:00:00")
...
class TestSCDETTS2BearishTS:
    def test_bias_is_bearish(self) -> None:
        result = detect_turtle_soup(_bearish_candles())
        assert result is not None
...
# ---------------------------------------------------------------------------
# SC-DET-TS-3: Doji ts_candle (close == open) → None (bullish scenario)
# ---------------------------------------------------------------------------


class TestSCDETTS3DojiReturnsNone:
    def test_returns_none_when_ts_candle_is_doji(self) -> None:
        ref = make_candle("2024-01-15 04:00:00", open=1.10050, high=1.10200, low=1.10000, close=1.10150)
        swing_low = make_candle("2024-01-15 08:00:00", open=1.10000, high=1.10100, low=1.09800, close=1.09900)
        recovery = make_candle("2024-01-15 12:00:00", open=1.09900, high=1.10050, low=1.09950, close=1.10000)
        # doji: close == open
        ts_doji = make_candle("2024-01-15 16:00:00", open=1.09900, high=1.10100, low=1.09700, close=1.09900)
        live = make_candle("2024-01-15 20:00:00")
        result = detect_turtle_soup([ref, swing_low, recovery, ts_doji, live])
        assert result is None


# ---------------------------------------------------------------------------
# SC-DET-TS-4: Doji ts_candle (close == open) → None (bearish scenario)
# ---------------------------------------------------------------------------


class TestSCDETTS4DojiReturnsNoneBearish:
    def test_returns_none_when_ts_candle_is_doji(self) -> None:
        ref = make_candle("2024-01-15 04:00:00", open=1.10000, high=1.10100, low=1.09900, close=1.10050)
        swing_high = make_candle("2024-01-15 08:00:00", open=1.10050, high=1.10300, low=1.10000, close=1.10200)
        recovery = make_candle("2024-01-15 12:00:00", open=1.10200, high=1.10200, low=1.10000, close=1.10100)
        # doji: close == open
        ts_doji = make_candle("2024-01-15 16:00:00", open=1.10200, high=1.10400, low=1.09950, close=1.10200)
        live = make_candle("2024-01-15 20:00:00")
        result = detect_turtle_soup([ref, swing_high, recovery, ts_doji, live])
        assert result is None
...
class TestSCDETTS7Smoke:
    def test_smoke_eurusd_4h(self) -> None:
